 ...
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not
   ...
   the License.
 */
export const latex_to_js = function (input: string): string {
  let init: Function,
    fraction: Function,
    ...
    cos: Function,
    tan: Function,
    sinCosTanFramework: Function,
    convert_others: Function;
  ...
  fraction = function (input: string): string {
    while (
      input.search(/\\frac\{(((?![\{\}]).)*)\}\{(((?![\{\}]).)*)\}/) >= 0
    ) {
      input = input.replace(
  ...
  square_root = function (input: string): string {
  ...
  nth_root = function (input: string): string {
    while (
      input.search(/\\sqrt\[(((?![\{\}]).)*)\]\{(((?![\{\}]).)*)\}/) >= 0
    ) {
      input = input.replace(
  ...
  nth_power = function (input: string): string {
  ...
  sin = function (input: string): string {
    return sinCosTanFramework("sin", input);
  };

  tan = function (input: string): string {
    return sinCosTanFramework("tan", input);
  };

  cos = function (input: string): string {
    return sinCosTanFramework("cos", input);
  };

  sinCosTanFramework = function (func: string, input: string): string {
    let pat1 = new RegExp(
      "\\\\" +
        func +
        "\\\\left\\(([0-9a-zA-Z\\.\\+\\*\\-\\\\\\(\\)\\/]+)\\\\right\\)"
    ...
    let pat2 = new RegExp("\\\\" + func + "([0-9a-zA-Z]+)");
    //eg:  /\\sin([0-9a-zA-Z]+)/:
  ...
  convert_others = function (func: string, input: string): string | undefined {
    let temp_input,
      arr,
      closest_pos,
      func_pos,
    ...
    if (closest_pos === undefined || closest_pos <= 0) {
      throw "syntax error";
    }
